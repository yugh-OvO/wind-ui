<template>
  <div class="container">
    <a-card class="general-card" title="奖品管理">
      ...
      >
        <template #columns>
          ...
          <a-table-column title="奖品数量" data-index="num" />
          <a-table-column title="剩余数量" data-index="surplusNum" />
...
<script lang="ts" setup>
  ...
  const renderData = ref<PrizeRecord[]>([]);
  ...
  const fetchData = async (
    params: PrizeParams = { ...pagination, ...formModel.value }
  ) => {
    setLoading(true);
    try {
      const { data } = await prizeList(params);
  ...
  const changeStatus = (id: number, status: number) => {
    changePrizeStatus(id, status).then(() => {
